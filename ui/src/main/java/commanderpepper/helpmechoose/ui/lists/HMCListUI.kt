...
fun HMCListUI(
    ...
    LazyColumn(
        ...
        items(items = hmcList) { item ->
            HMCListRowUI(hmcItem = item, onHMCClick = onHMCClick, onSortClick = onSortClick, onDeleteClick = onDeleteClick)
        }
    }
}
...
fun HMCListRowUI(
    ...
    Card {
        Row(
            ...
            Image(
                modifier = Modifier.clickable { onSortClick(hmcItem.id) },
                painter = painterResource(id = R.drawable.ic_sort),
                colorFilter = ColorFilter.tint(Color.Cyan),
                contentDescription = "Sort HMC List"
            )
        }
    }
}
